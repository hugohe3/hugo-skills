...
| `doc_to_md.py`（docx / html / epub / ipynb / pandoc） | 无显式清理（mammoth/nbconvert/ebooklib 已经是忠实转换；html 路径去除 `<head>/<style>/<script>`，视为必要而非启发式）；DOCX 会额外保留可读 Office Math fallback 与 EMF / WMF 资产 |
